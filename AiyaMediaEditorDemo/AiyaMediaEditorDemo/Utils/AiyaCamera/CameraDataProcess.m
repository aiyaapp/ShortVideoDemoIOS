...
- (CMSampleBufferRef)process:(CMSampleBufferRef)sampleBuffer{
    runSynchronouslyOnOpenglHelperContextQueue(^{
        ...
        if (self.renderSuspended) {
            NSLog(@"process has stoped");
            outputSampleBuffer = NULL;
            return;
        }
...
- (void)destroyInputProgram{
    runSynchronouslyOnOpenglHelperContextQueue(^{
        [glHelper useAsCurrentContext];
        
        if (inputProgram) {
            glDeleteProgram(inputProgram);
            inputProgram = 0;
        }
    });
}
...
- (void)releaseGLResources{
    [self destroyInputProgram];
    [self destroyInputFramebuffer];
    [self destroyOutputProgram];
