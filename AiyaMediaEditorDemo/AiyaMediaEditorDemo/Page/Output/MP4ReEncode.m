    ...
    if (success){
        ...
        if (assetAudioTrack){
            ...
            NSDictionary *compressionAudioSettings = @{
                                                       ...
                                                       AVEncoderBitRateKey   : [NSNumber numberWithInteger:_audioBitrate],
        ...
        if (assetVideoTrack){
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(MP4ReEncodeVideoParamWithNaturalSize:preferredTransform:)]) {
            ...
            [self.assetWriterVideoInput requestMediaDataWhenReadyOnQueue:_rwVideoSerializationQueue usingBlock:^{
                ...
                while ([self.assetWriterVideoInput isReadyForMoreMediaData] && !completedOrFailed){
                    ...
                    if (sampleBuffer != NULL){
                        ...
                        if(self.delegate && [self.delegate respondsToSelector:@selector(MP4ReEncodeProcessVideoSampleBuffer:)]){
...
- (void)readingAndWritingDidFinishSuccessfully:(BOOL)success withError:(NSError *)error
{
    if (!success){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"转换失败 ERROR : %@",[error description]);
            
            if (self.delegate && [self.delegate respondsToSelector:@selector(MP4ReEncodeFinish:)]) {
